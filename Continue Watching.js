...
(function () {
  ...
  function startPlugin() {
    ...
    console.log('[ContinueWatch] Версия: 1.2 Final (без рекламы)');
    console.log('[ContinueWatch] ========================================');
    ...
    Lampa.Listener.follow('full', function (e) {
      ...
      // Создаем кнопку
      var button = $('<div class="full-start__button selector button--continue-watch" style="position: relative;">' + '<svg viewBox="0 0 24 24" width="24" height="24" fill="none">' + '<path d="M8 5v14l11-7L8 5z" fill="currentColor"/>' + '<circle cx="12" cy="12" r="10.5" stroke="currentColor" stroke-width="1.5" fill="none" ' + 'stroke-dasharray="' + (percent * 65.97 / 100).toFixed(2) + ' 65.97" transform="rotate(-90 12 12)"/>' + '</svg>' + '<span>Продолжить ' + percent + '%</span>' + '<div style="position: absolute; bottom: 2px; left: 50%; transform: translateX(-50%); font-size: 10px; opacity: 0.7;">' + timeStr + '</div>' + '</div>');
      ...
      button.on('hover:enter', function () {
        ...
        if (savedUrl) {
          ...
          console.log('[ContinueWatch] Отключение рекламы...');
          try {
            // ✅ ОТКЛЮЧАЕМ РЕКЛАМУ ВРЕМЕННО
            var originalPrerollShow = null;
            if (typeof Lampa.Preroll !== 'undefined' && Lampa.Preroll.show) {
              originalPrerollShow = Lampa.Preroll.show;

              // Подменяем функцию показа рекламы
              Lampa.Preroll.show = function (data, callback) {
                console.log('[ContinueWatch] 🚫 Реклама пропущена');
                // Сразу вызываем callback без показа рекламы
                if (callback) callback();
              };
              console.log('[ContinueWatch] ✓ Preroll.show подменен');
            }

            // Запускаем плеер
            Lampa.Player.play(playerData);
            console.log('[ContinueWatch] ✅ Плеер запущен без рекламы!');

            // Восстанавливаем оригинальную функцию через 500ms
            if (originalPrerollShow) {
              setTimeout(function () {
                Lampa.Preroll.show = originalPrerollShow;
                console.log('[ContinueWatch] ✓ Preroll.show восстановлен');
              }, 500);
            }
          } catch (err) {
    ...
    console.log('[ContinueWatch] Функции:');
    console.log('[ContinueWatch]   ✓ Создание кнопки "Продолжить" на карточке');
    console.log('[ContinueWatch]   ✓ Автосохранение прогресса каждые 10 сек');
    console.log('[ContinueWatch]   ✓ Финальное сохранение при закрытии');
    console.log('[ContinueWatch]   ✓ Отключение рекламы через Preroll.show');
