...
(function () {
  ...
  // ========== AUDIO VISUALIZER (ПОРЯДОК ИСПРАВЛЕН) ==========
  var AudioVisualizer = {
    context: null,
    analyser: null,
    source: null,
    ...
    init: function(videoElement, container) {
      try {
        ...
        // ✅ КРИТИЧНО: Установить isActive = true СРАЗУ
        this.isActive = true;
        ...
        console.log('[AudioVisualizer] ✅ Init complete, isActive:', this.isActive, 'WebAudio:', this.useWebAudio);
        return true;
      } catch(e) {
    ...
    createVisualizer: function(container) {
      console.log('[AudioVisualizer] Creating elements...');
      ...
      console.log('[AudioVisualizer] ✅ Created, canvas:', canvas.width, 'x', canvas.height);
    },
    ...
    initWebAudio: function(videoElement) {
      ...
      console.log('[AudioVisualizer] Buffer:', this.bufferLength);
      ...
      console.log('[AudioVisualizer] ✅ WebAudio ready, isActive:', this.isActive);
    ...
    startWebAudioVisualization: function() {
      ...
      console.log('[AudioVisualizer] 🎬 Starting draw loop, isActive:', self.isActive);

      function draw() {
        if (!self.isActive || !self.analyser || !self.canvasCtx) {
          console.log('[AudioVisualizer] ⛔ Draw stopped, isActive:', self.isActive);
          return;
        }
        ...
        if (self.frameCount % 60 === 0) {
          var sum = 0;
          for(var j = 0; j < self.dataArray.length; j++) {
            sum += self.dataArray[j];
          }
          var avg = Math.round(sum / self.dataArray.length);
          console.log('[AudioVisualizer] 🎵 Frame', self.frameCount, 'avg:', avg);
        }

        // Очистка
        self.canvasCtx.fillStyle = 'rgba(30, 30, 45, 0.3)';
        self.canvasCtx.fillRect(0, 0, self.canvas.width, self.canvas.height);

        var barWidth = (self.canvas.width / self.bufferLength) * 2;
        var x = 0;

        for(var i = 0; i < self.bufferLength; i++) {
          var barHeight = (self.dataArray[i] / 255) * self.canvas.height * 0.9;

          var gradient = self.canvasCtx.createLinearGradient(
            0, self.canvas.height - barHeight, 0, self.canvas.height
          );
          gradient.addColorStop(0, 'rgba(100, 180, 255, 0.9)');
          gradient.addColorStop(0.5, 'rgba(60, 140, 255, 0.8)');
          gradient.addColorStop(1, 'rgba(40, 100, 220, 0.7)');

          self.canvasCtx.fillStyle = gradient;
          self.canvasCtx.fillRect(x, self.canvas.height - barHeight, barWidth - 3, barHeight);
    ...
    initCSSWave: function() {
      ...
      for(var i = 0; i < 25; i++) {
        ...
        bar.setAttribute('style',
          'width: 6px; min-height: 20px; border-radius: 3px; ' +
          'background: linear-gradient(to top, rgb(40,100,220), rgb(60,140,255), rgb(100,180,255)); ' +
    ...
    toggle: function(isPlaying) {
      ...
      console.log('[AudioVisualizer] Toggle:', isPlaying ? 'PLAY' : 'PAUSE');
    ...
    destroy: function() {
      console.log('[AudioVisualizer] 🗑️ Destroying...');
      this.isActive = false;
  ...
  styles.textContent = '@keyframes wave-anim { 0% { height: 30%; } 100% { height: 100%; } }';
  ...
  function integrateVisualizer() {
    ...
    function tryInitialize() {
      ...
      var checkInterval = setInterval(function() {
        attempts++;
        console.log('[MusicSearch] 🔄 Attempt', attempts, '/15');
        ...
        if (video && !visualizerInstance) {
          ...
          if (ok) {
            console.log('[MusicSearch] ✅✅✅ VISUALIZER IS RUNNING! ✅✅✅');
    ...
    Lampa.Player.listener.follow('start', function() {
      console.log('[MusicSearch] 🎬 START');
      setTimeout(tryInitialize, 500);
    });

    Lampa.Player.listener.follow('destroy', function() {
      console.log('[MusicSearch] 🗑️ DESTROY');
