...
async function testSource(index, btn) {
  ...
  try {
    await api('/ops/sources/test', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload) });
    btn.textContent = 'ОК';
...
async function runComponentAction(component, action, btn) {
  btn.disabled = true;
  try {
    const result = await api(`/ops/components/${component}/${action}`, { method: 'POST' });
...
async function runSystemAction(action, btn) {
  btn.disabled = true;
  try {
    const res = await api(`/ops/system/${action}`, { method: 'POST' });
...
async function save() {
  ...
  try {
    await api('/init', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(state) });
  } catch {
