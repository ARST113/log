    ...
    Lampa.Listener.follow('full', function (e) {
      ...
      function getRate(selector) {
        try {
          ...
          var docValDiv = docEl && docValDiv.querySelector('div');
...
(function () {
  ...
  function renderBadge(node, rating, source) {
    ...
    host.shadowRoot.innerHTML = 
      '<style>' +
        '.b{display:inline-flex;align-items:center;gap:4px;padding:4px 6px;border-radius:6px;' +
        'font:600 13px/1 -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Ubuntu,Arial,sans-serif;' +
        'color:#fff;pointer-events:none;opacity:0;transform:scale(.95);transition:all .25s ease;background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);}' +
        '.b.show{opacity:1;transform:scale(1);}' +
        '.b.tmdb{border:1px solid rgba(255,255,255,0.2);}' +
        '.b.kp{background:rgba(232,93,0,0.9);}' +
        '.b.imdb{background:rgba(245,197,24,0.9);color:#000;}' +
        '.logo{display:inline-block;background-size:contain;background-repeat:no-repeat;background-position:center;flex-shrink:0;}' +
        '.b.tmdb .logo{width:42px;height:16px;}' +
        '.b.kp   .logo{width:16px;height:16px;}' +
        '.b.imdb .logo{width:40px;height:16px;}' +
        '.value{flex-shrink:0;letter-spacing:-0.2px;}' +
      '</style>' +
      '<div class="b ' + source + '">' +
