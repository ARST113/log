(function() {
  ...
  var hostkey = 'http://lampac.fun'.replace('http://', '').replace('https://', '');
  ...
  window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
    if (!window.rch_nws[hostkey].startTypeInvoke) {
      ...
      if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
        ...
        net.silent('http://lampac.fun'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
          check(true);
        }, function() {
          check(false);
        }, false, {
    ...
    window.rch_nws[hostkey].typeInvoke('http://lampac.fun', function() {
      ...
      client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
        ...
        function sendResult(uri, html) {
          $.ajax({
            url: 'http://lampac.fun/rch/' + uri + '?id=' + rchId,
            type: 'POST',
            data: html,
            async: true,
            cache: false,
    ...
    this.draw = function(items) {
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          var
            episode = serial && episodes.length && !params.similars ? episodes.find(function(e) {
          ...
          element.mark = function() {
            ...
            var voice_name_text = choice.voice_name || element.voice_name ||
              element.title;
