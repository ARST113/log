(function () {
    ...
    var FIX_VERSION = 'series-meta-integrated-v1';
    ...
    function streamIdentity(url) {
        ...
        return [parsed.torrent_link || '', parsed.file_index !== undefined ? parsed.file_index : '', parsed.file_name || ''].join('|');
    }
    ...
    function readSeriesFields(data) {
        ...
        if (!season || !episode) {
            var fields = [data.title, data.name, data.episode_title, data.file_name, data.filename, data.path, data.path_human, data.folder_name, data.subtitle, data.label, data.text, data.info];
    ...
    function installSeriesMetaPatch() {
        ...
        noty('series meta patch installed');
        log('integrated patch installed', FIX_VERSION);
        return true;
    }

    function waitPatchInstall() {
        var started = Date.now ? Date.now() : new Date().getTime();
        var timer = setInterval(function () {
            if (installSeriesMetaPatch()) {
                clearInterval(timer);
                return;
            }
            ...
            if (now - started > 30000) {
                clearInterval(timer);
                warn('series meta patch install timeout');
            }
        }, 300);
    }
    ...
    function loadCore() {
        ...
        script.src = CORE_URL;
        script.async = true;
        script.onload = waitPatchInstall;
        script.onerror = function () {
            warn('failed to load core Debuger.js', CORE_URL);
            noty('core load failed');
        };
